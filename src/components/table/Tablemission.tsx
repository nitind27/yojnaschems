...
export default function Tablemission({ data, columns, Button }: any) {
    const [searchQuery, setSearchQuery] = useState("");
    const [filterStatus, setFilterStatus] = useState("");
    const [datestatus, setDatestatus] = useState(""); // State for selected date
    const [currentPageIndex, setCurrentPageIndex] = useState(0); // State for current page index
    const t = useTranslations("IndexPage");

    // Filter data based on search query, status, and date
    const filteredData = useMemo(() => {
        return data.filter((row: any) => {
            ...
            const matchesStatus = filterStatus === "" || row.subject === filterStatus;

            // Check if the row's date matches the selected date
            const matchesDate = datestatus === "" || row.testdate === datestatus;

            return matchesSearch && matchesStatus && matchesDate; // Combine all filters
        });
    }, [data, searchQuery, filterStatus, datestatus]); // Add datestatus to dependencies
    ...
    const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        setDatestatus(e.target.value); // Update the date state
        // Do not reset current page index here
    };
            ...
            <div className="row mb-3 align-items-center" style={{ display: "flex" }}>
                ...
                <div className="col-auto">
                    <input 
                        type="date" 
                        className="form-select" 
                        onChange={handleDateChange} // Use new handler for date change
                    />
                </div>
